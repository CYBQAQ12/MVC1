...
namespace WebApplication4.Models
{
    public class Chengji
    {
        ...
        [StringLength(50, MinimumLength = 1, ErrorMessage = "请输入 4 到 50 字符以内的【课程名称】。")]
        [RegularExpression(@"^[\u4e00-\u9fa5\a-zA-Z0-9]*$", ErrorMessage = "请输入由【字母或数字】组成的课程名称。")]
        [DataType(DataType.Text)]
        public string Kechengmingcheng { get; set; }

        [Display(Name = "课程成绩")]
        [Required(ErrorMessage = "【课程成绩】不能为空白")]
        [RegularExpression(@"^[0-9.-]*$", ErrorMessage = "请输入由【数字】组成的课程成绩。")]
        [DataType(DataType.Text)]
        public Single Kechengchengji { get; set; }
    }
}